...
Replace `your-gateway:8088` with your gateway address.
...
curl -u admin:password \
  "http://your-gateway:8088/data/camera-driver/snapshot?device=FrontDoor&profile=000"
```
...
The module supports two ways of connecting a camera: **ONVIF** (Profile S/T —
auto-discovers stream/snapshot URLs, media profiles, PTZ, device info) and
**direct URLs** (RTSP/MJPEG/snapshot entered by hand, no ONVIF required).

Cameras with proper ONVIF Profile S certification (Axis, Hikvision, Dahua,
Hanwha Techwin, and similar enterprise brands) work across all ONVIF
features. Some manufacturers — notably Reolink and other budget cameras —
claim ONVIF support but do not correctly implement `GetSnapshotUri`, so the
snapshot endpoint returns an error for those devices even though the camera
is otherwise connected. RTSP streaming is unaffected, because it does not
depend on that ONVIF call.

**If HTTP snapshots fail on an ONVIF camera:**

- Use the device's RTSP stream instead — it's in the OPC-UA tag
  `[default]OPC UA/<DeviceName>/Profiles/<ProfileName>/StreamUri` and works
  regardless of snapshot support.
- Or switch that device to direct URLs and supply the camera's own snapshot
  URL, bypassing the ONVIF call entirely.

**Checking a camera before relying on ONVIF:** create the device, then check
OPC UA Tag Browser for `[DeviceName]/Profiles/*` — at least one profile with a
populated `StreamUri` confirms ONVIF discovery is working; a populated
`SnapshotUri` does not guarantee the snapshot endpoint will actually succeed
against that camera.
