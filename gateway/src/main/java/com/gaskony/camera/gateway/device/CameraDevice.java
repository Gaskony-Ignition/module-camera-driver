...
public class CameraDevice extends ManagedAddressSpaceWithLifecycle implements Device {
     ...
     *
     * The probe ladder iterates ~22 paths per camera, so the timeout
     * dominates worst-case startup latency. 1 s is plenty for cameras on a
     * LAN; unreachable hosts fail fast, and the probe runs on a background
     * thread anyway (so a slow camera never blocks device startup).
     */
    ...
    // Device info, profiles, and per-profile stream/snapshot URIs are cached
    // rather than fetched live per /devices request — each device's full set
    // is ~8 SOAP round trips, and several devices pointed at one camera would
    // otherwise serialise dozens of calls against it per list request. These
    ...
    private void onStartup() {
        ...
        // Register and return immediately. Probing/ONVIF/address-space build
        // can take seconds-to-tens-of-seconds (especially on unreachable
        // cameras); doing it synchronously would serialise every device's
        // transition to RUNNING and stall the OPC-UA driver subsystem with N
        // cameras.
        //
        // Register the device early so handlers can find it during probing,
    ...
    private void performStartup() throws Exception {
        ...
        // User-configured URL overrides have highest priority
        ...
        // Try ONVIF first for PTZ control and URL discovery
        if (onvifEnabled) {
            deviceStatus = DeviceStatus.DISCOVERING.displayName();
            probeOnvif(ipAddress, port, username, password, useHttps, timeout);
        }

        // Fill URLs from ONVIF discovery (if available)
        ...
        // Auto-discover streams by probing the camera (for URLs not yet found)
        if (rtspEnabled && effectiveRtspUrl == null) {
            deviceStatus = DeviceStatus.DISCOVERING.displayName();
        ...
        // If RTSP still not found, build a default URL from the IP
        ...
        // Create GenericCameraClient for snapshot/MJPEG via HTTP
        ...
        // Register RTSP stream with go2rtc
